...
const double L=20.0;// width and depth of the surface
const int M=101; // grid size is M x M
const int Nc=30; // number of cells
const double dt=0.005;//0.0001; // time step
const int Nt=5000;//10000;//100000; //70000; // number of time steps
...
char ch[80];
FILE *substrate;
sprintf(ch,"numerical_results/substrate_density.txt");
substrate=fopen(ch,"w");
FILE *cells;
sprintf(ch,"numerical_results/cells.txt");
cells=fopen(ch,"w");
FILE *loads;
sprintf(ch,"numerical_results/loads.txt");
loads=fopen(ch,"w");
FILE *chemical_file;
sprintf(ch,"numerical_results/chemical.txt");
chemical_file=fopen(ch,"w");
FILE *displacement_magnitude;
sprintf(ch,"numerical_results/displ.txt");
...
for (it=0;it<Nt;it++)
{
    ...
    for (i=0;i<M;i++)
    {
        for (j=0;j<M;j++)
        {
            ...
            // update density
            rho[i][j]=rho_old[i][j]+dt*(-gamma_2*chemical[i][j]*rho_old[i][j]);
        }
    }
